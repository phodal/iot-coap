...
void coap_dump_hex(const uint8 *buf, size_t buflen, bool bare)
{
    if (bare)
    {
        while(buflen--)
...
void coap_dump_str(const uint8 *buf, size_t buflen)
{
	while (buflen--)
		printf("%c", *buf++);
}
...
int coap_parseHeader(coap_header_t *hdr, const uint8 *buf, size_t buflen)
{
    if (buflen < 4)
...
int coap_parseToken(coap_buffer_t *tokbuf, const coap_header_t *hdr, const uint8 *buf, size_t buflen)
{
    if (hdr->tkl == 0)
    {
        tokbuf->p = NULL;
...
// advances p
int coap_parseOption(coap_option_t *option, uint16 *running_delta, const uint8 **buf, size_t buflen)
{
    const uint8 *p = *buf;
    uint8 headlen = 1;
    uint16 len, delta;
...
int coap_parseOptionsAndPayload(coap_option_t *options, uint8 *numOptions, coap_buffer_t *payload, const coap_header_t *hdr, const uint8 *buf, size_t buflen)
{
    size_t optionIndex = 0;
    uint16 delta = 0;
    const uint8 *p = buf + 4 + hdr->tkl;
    const uint8 *end = buf + buflen;
    int rc;
    if (p > end)
        return COAP_ERR_OPTION_OVERRUNS_PACKET;   // out of bounds

    //coap_dump_hex(p, end - p);
...
void coap_dumpOptions(coap_option_t *opts, size_t numopt)
{
    ...
    for (i=0;i<numopt;i++)
    {
		...
		coap_dump_str(opts[i].buf.p, opts[i].buf.len);
        printf(" ]\n");
    }
}
#endif
...
void coap_dumpPacket(coap_packet_t *pkt)
{
    ...
    printf("Payload: ");
    coap_dump_hex(pkt->payload.p, pkt->payload.len, true);
    printf("\n");
}
#endif
...
int coap_parse(coap_packet_t *pkt, const uint8 *buf, size_t buflen)
{
    ...
    // coap_dump_hex(buf, buflen, false);
...
int coap_make_response(coap_rw_buffer_t *scratch, coap_packet_t *pkt, const uint8 *content, size_t content_len, uint8 msgid_hi, uint8 msgid_lo, const coap_buffer_t* tok, coap_code_t rspcode, coap_content_type_t content_type)
{
    ...
    if (scratch->len < 2)
        return COAP_ERR_BUFFER_TOO_SMALL;
    scratch->p[0] = ((uint16)content_type & 0xFF00) >> 8;
    scratch->p[1] = ((uint16)content_type & 0x00FF);
...
next:
    ...
    coap_make_response(scratch, outpkt, NULL, 0, inpkt->hdr.id[0], inpkt->hdr.id[1], &inpkt->tok, COAP_CODE_NOT_FOUND, COAP_CONTENTTYPE_NONE);
